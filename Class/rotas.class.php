...
class Rotas{
	...
	function __construct(){
		...
		$this->Local = ( $this->Local ? $this->Local: 'login');
	...
	private function checkLink(){
		...
		if(!preg_match( '/^[0-9]*$/' , $this->Local[0] ) ){
			...
			if(!file_exists(end($this->Local) . '.php')){

				// VERIFICA SE O ARQUIVO PHP DO INDICE 0 NÃO EXISTE. 
				if(!file_exists($this->Local[0] . '.php')){
					...
					if(isset($this->Local[1]) && !empty($this->Local[1])){
						...
						if(!preg_match( '/^[0-9]*$/' , $this->Local[1] ) ){
							...
							if(file_exists($this->Local[0] . '/' . $this->Local[1] . '.php')){
							...
							if(file_exists($this->Local[0] . '/home.php'))
							{
								$this->Path = $this->Local[0];
								$this->File = 'home';
								$this->Link = ( isset($this->Local[2]) ? $this->Local[2] : null);
						...
						if(file_exists($this->Local[0] .  '/login.php')){
							$this->Path = $this->Local[0];
							$this->File = 'login';
							$this->Link = ( isset($this->Local[2]) ? $this->Local[2] : null); 
